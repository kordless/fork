//! DNS handler — UDP queries (A/AAAA/MX/TXT/NS/CNAME/SOA/PTR).
//!
//! URL forms:
//!   dns://example.com
//!   dns://example.com/MX
//!   dns://example.com?type=TXT&server=1.1.1.1
    ...
    async fn fetch(&self, url: &Url) -> Result<ProtocolPayload> {
        ...
        for server in &servers {
            match query_server(*server, &query).await {
                Ok(raw) => {
                    let text = format_message(name, qtype, &raw)?;
                    return Ok(ProtocolPayload {
                        raw_bytes: raw,
                        content_type: Some("application/dns-message".into()),
                        extracted_text: text,
                        status: 200,
                        title: Some(format!("{name} {}", type_name(qtype))),
                        links: vec![],
...
fn query_type(url: &Url) -> Result<u16> {
    if let Some((_, v)) = url.query_pairs().find(|(k, _)| k == "type") {
        return parse_qtype(&v);
    }
...
fn resolver_addrs(url: &Url) -> Result<Vec<SocketAddr>> {
    if let Some((_, v)) = url.query_pairs().find(|(k, _)| k == "server") {
        ...
        return Ok(vec![with_port.parse().with_context(|| format!("bad DNS server {with_port}"))?]);
    }
    DEFAULT_SERVERS
        .iter()
...
pub(crate) fn encode_query(id: u16, name: &str, qtype: u16) -> Result<Vec<u8>> {
    let mut buf = Vec::new();
...
fn format_message(qname: &str, qtype: u16, msg: &[u8]) -> Result<String> {
    if msg.len() < 12 {
    ...
    for (label, count) in sections {
        ...
        for _ in 0..count {
            ...
            out.push_str(&format!(
                "{name}. {ttl} {} {} {rendered}\n",
                if class == CLASS_IN { "IN" } else { "CLASS" },
                type_name(typ)
            ));
        }
    }
...
mod tests {
    ...
    #[test]
    fn parses_type_from_path_and_query() {
        let u = Url::parse("dns://example.com/MX").unwrap();
        assert_eq!(query_type(&u).unwrap(), TYPE_MX);
        let u = Url::parse("dns://example.com?type=aaaa").unwrap();
        assert_eq!(query_type(&u).unwrap(), TYPE_AAAA);
    }

    #[test]
    fn decodes_compressed_name() {
        // example.com + pointer back to it
    ...
    #[tokio::test]
    async fn fetches_from_local_udp_server() {
        ...
        tokio::spawn(async move {
            ...
            resp[6] = 0;
            resp[7] = 1; // one answer
            // answer: pointer to QNAME, A, IN, ttl=60, rdlen=4, 1.2.3.4
            resp.extend_from_slice(&[0xC0, 12, 0, 1, 0, 1, 0, 0, 0, 60, 0, 4, 1, 2, 3, 4]);
            let _ = sock.send_to(&resp, peer).await;
        });

        let url = Url::parse(&format!("dns://example.com?server={addr}")).unwrap();
        let payload = DnsHandler.fetch(&url).await.unwrap();
        assert_eq!(payload.status, 200);
        assert!(payload.extracted_text.contains("1.2.3.4"));
        assert!(payload.extracted_text.contains("ANSWER"));
    }
}
